...
@Component({
  selector: 'app-member-clubs-component',
  imports: [
    Navbar,
    RouterLink
  ],
  templateUrl: './member-clubs-component.html',
  styleUrl: './member-clubs-component.css',
})
export class MemberClubsComponent { clubs = signal<Clubs[]>([]);
  loading = signal(true);
  error = signal('');
   ...
   */
  loadClubs(): void {
    this.clubService.getClubsByMemberId().subscribe({
      next: (clubs) => {
